...
export default function AdminProjectManagement() {
      ...
      supabase
        ...
        .select(`
          *,
          ...
          quotation:ict_quotations(id,quotation_no,status)
        `)
  ...
  const convertedQuotationIds = useMemo(
    () =>
      new Set(
        projects
          .map((project) => project.quotation_id)
          .filter(Boolean)
      ),
    [projects]
  );

  const acceptedWaiting = useMemo(
    () =>
      quotations.filter(
        (quotation) =>
          !convertedQuotationIds.has(quotation.id)
      ),
    [quotations, convertedQuotationIds]
  );
  ...
  async function convertAcceptedQuotation(quotationId) {
    try {
      setConvertingId(quotationId);
      setMessage({ type: "", text: "" });

      const { data: projectId, error } =
        await supabase.rpc(
      ...
      setMessage({
        type: "success",
        text: "تم تحويل عرض السعر المقبول إلى مشروع تشغيلي بنجاح.",
      });
      ...
      if (projectId) {
        window.location.href =
          `/admin/project-commercial?project=${projectId}`;
      }
    } catch (error) {
      setMessage({
        type: "error",
        text:
          error.message ||
          "تعذر تحويل عرض السعر إلى مشروع.",
      });
    } finally {
      setConvertingId("");
    }
    ...
    try {
      ...
      if (form.quotation_id) {
        ...
        if (
          ...
          throw new Error(
            "لا يمكن ربط المشروع إلا بعرض سعر مقبول."
          );
        }

        if (
          convertedQuotationIds.has(
            form.quotation_id
          )
        ) {
          throw new Error(
            "عرض السعر مرتبط بمشروع بالفعل."
          );
        }
      }
      ...
      <div className="mx-auto max-w-7xl">
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div>
            ...
            <p className="mt-3 text-slate-600">
              تحويل العروض المقبولة إلى مشاريع فعلية ومتابعة التنفيذ.
            </p>
          </div>
        ...
        <section className="mt-8 rounded-3xl border border-green-200 bg-green-50 p-6 shadow-sm">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div>
              <h2 className="text-2xl font-black text-green-950">
                عروض مقبولة بانتظار التحويل
              </h2>

              <p className="mt-2 text-green-800">
                يظهر هنا فقط عرض السعر الذي قبله العميل ولم يتحول إلى مشروع بعد.
              </p>
            </div>

            <span className="rounded-full bg-white px-4 py-2 font-black text-green-800">
              {acceptedWaiting.length}
            </span>
          </div>
          ...
          <div className="mt-5 grid gap-4 lg:grid-cols-2">
                ...
                >
                  <div className="flex flex-wrap items-start justify-between gap-4">
                    <div>
                      ...
                      <h3 className="mt-2 text-xl font-black text-[#071d49]">
                        {quotation.subject ||
                          "عرض سعر"}
                      </h3>
                    ...
                    <span className="inline-flex items-center gap-2 rounded-full bg-green-100 px-3 py-2 font-black text-green-800">
                      <FaCheck />
                      مقبول
                    </span>
                  </div>
                  ...
                  <button
                    ...
                    className="mt-5 inline-flex items-center gap-2 rounded-xl bg-[#ff7417] px-5 py-3 font-black text-white disabled:opacity-60"
                  >
                    <FaArrowRight />
                    {convertingId ===
                    quotation.id
            ...
            {!acceptedWaiting.length && (
              <div className="rounded-2xl bg-white p-6 text-center text-slate-500 lg:col-span-2">
                لا توجد عروض مقبولة بانتظار التحويل.
              </div>
            )}
          </div>
        </section>
        ...
        >
          ...
          <p className="mt-2 text-sm text-slate-500">
            استخدم هذا النموذج فقط للمشاريع المباشرة أو الحالات الاستثنائية.
          </p>
          ...
          <button
            type="submit"
            disabled={saving}
            className="mt-6 inline-flex items-center gap-2 rounded-xl bg-[#123878] px-6 py-3 font-black text-white disabled:opacity-60"
          >
            <FaPlus />
            {saving
              ? "جارٍ الحفظ..."
