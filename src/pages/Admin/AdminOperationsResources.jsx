...
export default function AdminOperationsResources({ mode = "all" }){
 ...
 const [warehouse,setWarehouse]=useState({name:"",location:""}),[item,setItem]=useState({name:"",sku:"",category:"",unit:"pcs",warehouse_id:"",supplier_id:"",quantity_on_hand:"0",reorder_level:"0",average_cost:"0",serial_tracked:false,status:"active",notes:""});
 ...
 async function load(){
  ...
  if(mode==="inventory"){
   ...
   const stockValue=inventoryBalances.reduce((sum,row)=>{
    ...
    return sum+(Number(row.quantity_on_hand||0)*Number(item?.average_cost||0));
   },0);
 ...
 return <div dir="rtl" className="erp-page"><div className="mx-auto max-w-[1500px]">
  ...
  <div className="mt-6 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">{mode==="inventory"&&<><K t="المستودعات" v={snap.warehouses}/><K t="أصناف المخزون" v={snap.inventory_items}/><K t="قيمة المخزون" v={snap.stock_value} m/><K t="Low Stock" v={snap.low_stock_items}/></>}{mode==="assets"&&<><K t="الأصول" v={snap.assets}/><K t="العهد النشطة" v={snap.assigned_assets}/></>}{mode==="attendance"&&<><K t="الموظفون" v={snap.active_employees}/><K t="الحضور اليوم" v={snap.present_today}/><K t="إجازة اليوم" v={snap.on_leave_today}/></>}{mode==="payroll"&&<><K t="الموظفون" v={snap.active_employees}/><K t="Payroll Value" v={snap.payroll_value} m/></>}</div>
   ...
   <form className="erp-card p-6" onSubmit={e=>{e.preventDefault();add("ict_inventory_items",{...item,warehouse_id:item.warehouse_id||null,supplier_id:item.supplier_id||null,quantity_on_hand:Number(item.quantity_on_hand||0),reorder_level:Number(item.reorder_level||0),average_cost:Number(item.average_cost||0),serial_tracked:Boolean(item.serial_tracked),notes:item.notes||null},"تم إنشاء الصنف.")}}><H i={<FaBoxesStacked/>} t="صنف مخزون"/><F l="اسم الصنف" v={item.name} c={v=>setItem(x=>({...x,name:v}))}/><F l="SKU" v={item.sku} c={v=>setItem(x=>({...x,sku:v}))}/><F l="متوسط التكلفة" type="number" v={item.average_cost} c={v=>setItem(x=>({...x,average_cost:v}))}/><F l="الكمية الحالية" type="number" v={item.quantity_on_hand} c={v=>setItem(x=>({...x,quantity_on_hand:v}))}/><F l="حد إعادة الطلب" type="number" v={item.reorder_level} c={v=>setItem(x=>({...x,reorder_level:v}))}/><label className="mt-4 flex items-center gap-3"><input type="checkbox" checked={item.serial_tracked} onChange={e=>setItem(x=>({...x,serial_tracked:e.target.checked}))}/><b>تتبع Serial</b></label><F l="ملاحظات" v={item.notes} c={v=>setItem(x=>({...x,notes:v}))}/><button className="erp-btn-primary mt-5"><FaPlus/>إضافة</button></form>
   ...
   <section className="erp-card p-6 xl:col-span-3 overflow-x-auto"><table className="w-full min-w-[850px]"><thead><tr><th className="p-3">المستودع</th><th>الصنف</th><th>On Hand</th><th>Reserved</th><th>Available</th><th>القيمة</th></tr></thead><tbody>{balances.map(b=>{const i=items.find(x=>x.id===b.item_id);return <tr key={b.id} className="border-t"><td className="p-3">{wh.find(x=>x.id===b.warehouse_id)?.name||"—"}</td><td>{i?.name||"—"}</td><td>{b.quantity_on_hand}</td><td>{b.quantity_reserved}</td><td className="font-black">{Number(b.quantity_on_hand)-Number(b.quantity_reserved)}</td><td>{money(Number(b.quantity_on_hand)*Number(i?.average_cost||0))}</td></tr>})}</tbody></table></section>
  </div>}
