...
export default function PortalResetPassword() {
  ...
  return (
    <div dir="rtl" className="flex min-h-screen items-center justify-center bg-[#03152f] px-4">
      <form onSubmit={submit} className="w-full max-w-md rounded-3xl bg-white p-8 shadow-2xl">
        ...
        {message && <div className="mt-6 rounded-2xl bg-green-50 p-4 text-green-800">{message}</div>}

        <label className="mt-7 block">
          <span className="mb-2 block font-bold">كلمة المرور الجديدة</span>
          <input type="password" value={password} required onChange={e=>setPassword(e.target.value)} className="form-input" />
        </label>

        <label className="mt-5 block">
          <span className="mb-2 block font-bold">تأكيد كلمة المرور</span>
          <input type="password" value={confirmPassword} required onChange={e=>setConfirmPassword(e.target.value)} className="form-input" />
        </label>
        ...
        <Link to="/" className="mt-4 block text-center font-bold text-slate-600">
          العودة إلى الموقع
        </Link>
      </form>
    </div>
